...
<instructions>
N1. RUN - one Bash call: `python -m taskcontract new {id}`. The skeleton is deliberately red: TC007 trips until a real intent is authored, so a fresh contract can never pass the gate vacuously. It names `entities` in one comment line and sets no value, so at ready TC017 trips until the author declares the terms the task operates on, or `[]` for none (ADR 0030).
