...
# An item line: its indent, its id, its status (a finding: its kind) in
# brackets, then the rest. A diagnostic line: its indent, `- `, its text.
ITEM = re.compile(r"^(?P<indent>(?:  )*)(?P<id>\S+) \[(?P<tag>[^\]]*)\](?P<rest>.*)$")
...
def _print(root, capsys):
    ...
    a dict of depth, id, tag, rest and the diagnostics printed under it;
    ...
    for text in out.splitlines():
        ...
        rows.append({"depth": len(item["indent"]) // 2, "id": item["id"], "tag": item["tag"],
                     "rest": item["rest"], "line": text, "diagnostics": []})
    return rows, out, err
...
def test_sc2_1_a_contract_shows_its_active_gates_then_the_next_gate_inactive(tmp_path, capsys):
    ...
    assert _row(rows, "ready/G1")["line"] == "  ready/G1 [to do] inactive | Requirements / Spec"
    ...
    assert _row(rows, "ready/G4")["line"] == "  ready/G4 [to do] | Pre-merge CI"
    assert _row(rows, "ready/G5")["line"] == "  ready/G5 [to do] inactive | Integration / System"
    ...
    for gate, page in pages:
        ...
        for base in (f"gates/{gate}", f"ready/{gate}"):
            ...
            for cid, name in conditions:
                ...
                # a condition line carries its status and its name, no evidence
                assert row["rest"] == f" | {name}", row["line"]
...
def test_sc2_1_an_inactive_gate_never_counts_toward_its_contracts_status(tmp_path, capsys):
    ...
    assert _row(rows, "ready")["line"].startswith("ready [done] at 1a2b3c4 | ")  # a closed contract
...
def test_sc2_2_a_done_condition_lists_no_diagnostic(tmp_path, capsys):
    ...
    assert lines[at + 1:at + 5] == [
        "    ready/G0/G0.1 [done] | Definition-of-ready",
        "    ready/G0/G0.2 [done] | Vocabulary coverage",
        "    ready/G0/G0.3 [done] | Unit confirmation",
        "  ready/G1 [to do] inactive | Requirements / Spec"]
...
def test_sc2_2_a_finding_that_names_a_condition_stands_once_under_it_for_the_repository(
    ...
    rows = _tree(root, capsys)
    assert _row(rows, "gates/G3")["line"] == "gates/G3 [to do] inactive | Implementation"
    ...
    assert _row(rows, "gates/G3/G3.1")["line"] == "  gates/G3/G3.1 [to do] | Formatter"
    assert _row(rows, "gates/G0/G0.2")["line"] == "  gates/G0/G0.2 [to do] | Vocabulary coverage"
    assert _row(rows, "gates/G3/G3.1/slow-loop")["line"] == (
        f"    gates/G3/G3.1/slow-loop [kind: gap] gate: G3.1 | {STATEMENT}")
...
def test_sc2_2_a_finding_that_names_its_gate_or_an_unlisted_condition_stands_after_the_conditions(
    ...
    assert _row(rows, "gates/G3/odd-one")["line"] == (
        f"  gates/G3/odd-one [kind: gap] gate: G3.9 | {STATEMENT}")
...
def test_sc2_2_the_query_prints_a_condition_as_it_prints_a_gate_without_diagnostics(
    ...
    expected = {
        "drafted/G0/G0.2": ("drafted/G0/G0.2 [to do]\n"
                            "summary: Vocabulary coverage\n"
                            f"page: {G0_PAGE}\n"),
        "short/G0/G0.1": ("short/G0/G0.1 [failed]\n"
                          "summary: Definition-of-ready\n"
                          f"page: {G0_PAGE}\n"),
        "ready/G1/G1.1": ("ready/G1/G1.1 [to do]\n"
                          "summary: Spec/schema linting\n"
                          "page: docs/gates/G1-requirements-spec.md\n"),
        "gates/G3/G3.1": ("gates/G3/G3.1 [to do]\n"
                          "summary: Formatter\n"
                          "page: docs/gates/G3-implementation.md\n"),
        "ready/G1": ("ready/G1 [to do] inactive\n"
                     "summary: Requirements / Spec\n"
                     "file: specs/ready/contract.yaml:1\n"
                     "page: docs/gates/G1-requirements-spec.md\n"),
        "gates/G3/G3.1/slow-loop": ("gates/G3/G3.1/slow-loop [kind: gap]\n"
                                    f"summary: {STATEMENT}\n"
                                    "gate: G3.1\n"
...
def _verdict_line(cid, status):
    return (f"  {cid}/G0 [{status}] via python -m taskcontract validate "
            f"specs/{cid}/contract.yaml --profile ready at no commit | Planning / Intake")
...
def test_usage_marks_its_project_tree_subsections_red_and_its_example_is_the_tree(
    ...
    example = part.split("```\n", 2)[1].splitlines()
    assert example[0].startswith("apply-discount [to do]")
    root = _repo(tmp_path)
    _term(root, "discount-code", "draft")
    _put(root, "apply-discount", _contract(
        "apply-discount", intent="Checkout applies one discount code per order.",
        entities=["discount-code"]))
    _, out, _ = _print(root, capsys)
    lines = out.splitlines()
    wanted = [text.replace(" at 1a2b3c4 ", " at no commit ") for text in example]
    assert wanted[0] in lines
    at = lines.index(wanted[0])
    assert lines[at:at + len(wanted)] == wanted  # the example, line for line
...
def _first(row):
    ...
    after the evidence, so without its links, doc and summary."""
    return f"{row['id']} [{row['tag']}]" + _LINKED.sub("", row["rest"].partition(" | ")[0])
