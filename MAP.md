...
| controlled language | Bounded interpretation for executed prose: set-ratified closed dictionary + field registry, `lang-check` door (CLnnn) + `lang-extract` census over contract prose fields, exempt ratchet for pre-arc contracts, base layer in the wheel | implemented (v1) | [docs/controlled-language.md](docs/controlled-language.md) |
| raw-request intake | The consumer's feature document is the raw request; intake produces the contract from it (section-to-field mapping, template additions, revision-table row as the linkage record); USAGE section lands with the demo intake | designed | [decisions/0026](decisions/0026-feature-document-as-raw-request.md) |
| specification interview | `/sdlc:product-specification-interview`, the feature document's writer, upstream of G0: the template's sections one question at a time, resumable state beside the document, an advisory readiness check, no-clobber output that names `/sdlc intake`; domain modules deferred | implemented (v1) | [decisions/0028](decisions/0028-specification-interview-skill.md) |
| playbook alignment | Anthropic's AI-native SDLC playbook crosswalked to the 56 conditions; six gaps close in two contracts: `playbook-guardrails` (the session hook, G4.12 `scope-check` with the `Contract:` trailer, REVIEW.md, the engineer's three questions at intake; shipped, 0.13.0) and `playbook-loop` (G9.5 re-scan, flow metrics, PO venue, eval refs; planned) | partial (v1) | [decisions/0027](decisions/0027-playbook-crosswalk-and-closures.md) |
| gate deep pages | Per-gate condition rosters on the fixed template, one page per gate; the registry `docs/gates.md` stays the one-line index | designed | [G0](docs/gates/G0-planning-intake.md), [G1](docs/gates/G1-requirements-spec.md), [G2](docs/gates/G2-design-architecture.md), [G3](docs/gates/G3-implementation.md), [G4](docs/gates/G4-pre-merge-ci.md), [G5](docs/gates/G5-integration-system.md), [G6](docs/gates/G6-uat-staging.md), [G7](docs/gates/G7-release-deploy.md), [G8](docs/gates/G8-operations.md), [G9](docs/gates/G9-maintenance.md), [G10](docs/gates/G10-retirement.md), [PL-DOC](docs/gates/PL-DOC-documentation.md), [PL-PIPE](docs/gates/PL-PIPE-pipeline-integrity.md) |
| work tree | `taskcontract tree`: the work as one tree derived at every run (gates with their findings, contracts with verdicts, units, the seven tasks, checks), six statuses with their evidence, a query face for one item, and the `--follow` pane with its `notify` edge; `taskcontract progress` records task steps and check runs in local files no gate reads | implemented (v1) | [decisions/0031](decisions/0031-the-work-is-one-derived-tree.md) |
