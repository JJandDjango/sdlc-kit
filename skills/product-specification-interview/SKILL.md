...
|---|---|---|
| `$1` | Feature slug matching `^[a-z][a-z0-9-]{2,63}$`; names the document and its state file | asked when absent |
| `$2` | State file to resume from | the flow looks for `REQUEST_{slug}_*.state.yaml` at the repo root |
</variables>

<context>
Runs in the consumer repo's Claude Code session; cwd is the repo root.
{skill-dir} is the directory holding this file:
  {skill-dir}/flows/opening.md    O1-O6: origin, id and title, seats, materials, path; writes the state file
...
root (the user confirms the path in O6), and beside it the state file
...
writes it at every section boundary.
...
<instructions>
0. DISPATCH on the arguments. PARSE `$1` as the slug; ASK for it when absent or when it fails the pattern. LOAD the state file: `$2` when given, else one Bash call `ls REQUEST_{slug}_*.state.yaml` at the repo root. Exactly one file resumes; none starts a new run; two or more means ASK which one (AskUserQuestion).
1. ROUTE by the state file's `phase`: none or `opening` - LOAD {skill-dir}/flows/opening.md and EXECUTE O1-O6; `sections` - LOAD {skill-dir}/flows/sections.md and EXECUTE from the step `next` names; `readiness` - LOAD {skill-dir}/flows/readiness.md and EXECUTE R1-R4; `output` - LOAD {skill-dir}/flows/output.md and EXECUTE W1-W5; `complete` - REPORT the document path and `/sdlc intake {path}`, then return.
...
<criteria>
- [ ] Dispatch honored: the slug parsed or asked; the state file found by `$2` or the `ls` pattern; one file resumes at `next`, none starts at O1, several ask.
